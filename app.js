...
const SUGESTOES_INICIAIS = [
  ...
  {servico:"Cerâmica", acaoTipo:"Substituir", pendencia:"Peça com som oco ou comprometida", texto:"Remover a peça comprometida, preparar a base e executar nova instalação, garantindo aderência, alinhamento e nivelamento adequados."},
  {servico:"Cerâmica", acaoTipo:"Rejuntar", pendencia:"Falta de rejunte", texto:"Executar o preenchimento das juntas com rejunte adequado, garantindo uniformidade, acabamento e preenchimento completo."},
  ...
  {servico:"Serralheria", acaoTipo:"Regularizar", pendencia:"Elemento desalinhado", texto:"Executar a regulagem e correção do elemento metálico, garantindo alinhamento, funcionamento e acabamento adequado."},
...
function buscarSugestoes(){
  ...
  const acao=$("pAcaoTipo").value; const pend=normalizar($("pPendencia").value);
  const filtradas=sugestoes.filter(x=>{
    const okServico=!servico || normalizar(x.servico)===normalizar(servico);
    const okAcao=!acao || normalizar(x.acaoTipo)===normalizar(acao);
    const texto=normalizar(`${x.pendencia} ${x.texto}`);
    const okPend=!pend || pend.split(/\s+/).filter(w=>w.length>2).some(w=>texto.includes(w));
    return okServico && okAcao && okPend;
  });
...
function renderPendencias(){
  ...
  $("pPendencia").value="";$("pAcao").value=item.acao||"";$("pPrioridade").value=item.prioridade||"Média";$("pStatus").value="Pendente";
  $("formPendencia").querySelector('button[type="submit"]').textContent="Salvar pendência";
  $("formPendencia").scrollIntoView({behavior:"smooth",block:"start"});
};
