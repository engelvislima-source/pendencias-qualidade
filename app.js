...
$("obra").addEventListener("change", e=>{
  $("obraOutraWrap").classList.toggle("hidden", e.target.value !== "__outra__");
});
...
$("formVistoria").addEventListener("submit", e=>{
  e.preventDefault();
  const obra = $("obra").value === "__outra__" ? $("obraOutra").value.trim() : $("obra").value;
...
$("pFoto").addEventListener("change", async e=>{
  const file=e.target.files?.[0]; if(!file){fotoAtual="";return;}
  fotoAtual = await compressImage(file, 1280, .72);
  $("previewFoto").src=fotoAtual;
...
$("formPendencia").addEventListener("submit", e=>{
  ...
  $("pPrioridade").value="Média"; $("pStatus").value="Pendente"; fotoAtual="";
  $("previewFoto").classList.add("hidden"); $("servicoOutroWrap").classList.add("hidden");
  renderPendencias();
});
...
function renderPendencias(){
  ...
  atual.pendencias.forEach(item=>{
    ...
    const img=node.querySelector(".thumb"); img.src=item.foto||"data:image/svg+xml;charset=UTF-8,"+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120"><rect width="100%" height="100%" fill="#eef2ef"/><text x="50%" y="52%" dominant-baseline="middle" text-anchor="middle" fill="#7c897f" font-size="14">Sem foto</text></svg>');
...
function renderResumo(){
  ...
  atual.pendencias.forEach((p,i)=>{
    ...
    el.innerHTML=`<img alt="" src="${p.foto||""}">
      <div>
    ...
    if(!p.foto) el.querySelector("img").style.display="none";
    list.appendChild(el);
  });
}
...
function renderHome(){
  ...
  vistorias.forEach(v=>{
    const d=document.createElement("div"); d.className="visit-card";
    d.innerHTML=`<div><strong>${escapeHtml(v.obra)}</strong><p>${fmtDate(v.data)} • ${v.pendencias.length} pendência(s) • ${escapeHtml(v.responsavel)}</p></div><button class="ghost">Abrir</button>`;
    d.querySelector("button").onclick=()=>{ atual=structuredClone(v); saveDraft(); carregarRegistro(); renderResumo(); show("resumoView"); }; box.appendChild(d);
  });
}
